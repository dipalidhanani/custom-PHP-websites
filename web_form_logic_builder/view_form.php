...
<div class="heading"><strong>View Journal Form</strong></div>
<div class="form-row"> <label>Journal Entry Title : </label>

        <input type="text" class="text" name="journal_entry_title" id="journal_entry_title"  />
</div>
<div class="form-row"> <label>Journal Entry Title :</label>
<span>
        <input type="radio" name="journal_entry_status" id="journal_entry_status" value="Open"  />Open<br />
        <input type="radio" name="journal_entry_status" id="journal_entry_status" value="Stalking"  />Stalking<br />
        <input type="radio" name="journal_entry_status" id="journal_entry_status" value="Closed"  />Closed
		</span>
</div>
     <script>
			$(function() {
...
<div class="form-row"> <label>Journal Date : </label>
        <input type="text" class="text" name="journal_entry_date" id="journal_entry_date" />
</div>
     <?php 
...
<div class="form-row"> <label><?php echo @$json_string_arr_single['label']; ?> : </label>
        <?php  for($j=0;$j<count($dropdown_options1);$j++){			
			$dropdown_options2 = (array)$dropdown_options1[$j]; 
			 ?>
             <input type="radio" name="<?php echo @$json_string_arr_single['cid']; ?>" id="<?php echo @$json_string_arr_single['cid']; ?>" value="<?php echo $dropdown_options2['label']; ?>" /><?php echo $dropdown_options2['label']; ?><br />       
        <?php } ?>       
</div>
        <?php } else if($json_string_arr_single['field_type'] == 'section_break'){ ?>
<div class="form-inner"><strong><?php echo @$json_string_arr_single['label']; ?></strong></div>
