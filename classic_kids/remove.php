...
u_Connect("cn");
	...
	$totaldelete=count($_REQUEST["removefromcart"]);

	if($_REQUEST["removefromcart"]!="")
	{
		foreach($_REQUEST["removefromcart"] as $value) 
		{ 
			$remid[$c]=$value;
			$rem=$remid[$c];
			unset($_SESSION['shopcart'][$rem]);		
		}
	}
	
	$count=$_SESSION['cartno'];
	$count=$count-$totaldelete;
