...
if($_REQUEST["process"]=="Add")
{
	...
	while($file=readdir($trackdir))
	{
		...
		if(is_dir("../colors|".$file))
		{
			if((strcmp(trim($file))==0))
			{
				$docf1=time()."_".$_FILES['colorfile']['name'];
				move_uploaded_file($_FILES["colorfile"]["tmp_name"],"../colors"."/".$docf1);
				$dir_exists_flag=1;
	...
	while($file=readdir($trackdir))
	{
		...
		if(is_dir("../colors|".$file))
		{
			if((strcmp(trim($file))==0))
			{
				...
				move_uploaded_file($_FILES["changefile"]["tmp_name"],"../colors"."/".$docf1);
				$dir_exists_flag=1;
				//echo "file added to directory";
			}
		}
