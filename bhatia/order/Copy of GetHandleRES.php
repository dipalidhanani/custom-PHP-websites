...
if($strResponseIPAdd == "221.134.101.174" || $strResponseIPAdd == "221.134.101.169")
{
	...
	{
			header('Location:https://order.bhatiamobile.com/FailedTRAN.php?message=ERROR IN PROCESSING PAYMENT 3 ('.$ErrorResult.' )&ME_TX_ID='.$METRANID.'&result=FAILED');
					exit();
	}
	...
	if ($ErrorTx == '')
		{
			...
			$recordsetchecktransid = mysql_query("select * from prod_order_mst 
			where order_invoice_Id='".$txmeid."' and order_payment_ID='".$pgtxnid."'");				
			
			if(mysql_num_rows($recordsetchecktransid)==0)
			{
					header('Location:https://order.bhatiamobile.com/FailedTRAN.php?message=ERROR IN PROCESSING PAYMENT 1 ('.$ErrorResult.' )&ME_TX_ID='.$txmeid.'&PGID='.$pgtxnid.'&result='.$txmessage);
					exit();
			}
		
			if($txamount!=$thisorderamount)
			{					
					header('Location:https://order.bhatiamobile.com/FailedTRAN.php?message=ERROR IN PROCESSING PAYMENT 2 ('.$ErrorResult.' )&ME_TX_ID='.$txmeid.'&result='.$txmessage);
					exit();
			}  
			
			$_SESSION["bhatia_payment_transaction_id"]=$txmeid;
			$_SESSION["bhatia_payment_result"]=$txmessage;	         
