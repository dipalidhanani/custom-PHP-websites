...
if($strResponseIPAdd == "221.134.101.174" || $strResponseIPAdd == "221.134.101.169")
{
	...
	if ($ErrorTx == '')
		...
		*/
		          ...
		          $REDIRECT = 'REDIRECT=http://www.bhatiamobile.com/hdfc/StatusTRAN.php?message=PAYMENT SUCESSFUL'.'&ME_TX_ID='.$getTxnTrackID;
			      echo $REDIRECT;
				...
				else
				{
				  ...
				  AND THEN REDIRECT CUSTOMER ON RESULT PAGE
				  */
					$REDIRECT = 'REDIRECT=http://www.bhatiamobile.com/hdfc/FailedTRAN.php?message=ERROR - PAYMENT FAILED IN DUAL VERIFICATION ('.$getTxnResult.' )&ME_TX_ID='.$txmeid;
					echo $REDIRECT;
			...
			else
			{
				...
				AND THEN REDIRECT CUSTOMER ON RESULT PAGE
				*/
				$REDIRECT = 'REDIRECT=http://www.bhatiamobile.com/hdfc/FailedTRAN.php?message=PAYMENT FAILED ('.$txmessage.' )&ME_TX_ID='.$txmeid;
				echo $REDIRECT;
	...
	else 
	{
				...
				*/ 
				...
				$REDIRECT = 'REDIRECT=http://www.bhatiamobile.com/hdfc/FailedTRAN.php?message=ERROR IN PROCESSING PAYMENT ('.$ErrorResult.' )&ME_TX_ID='.$METRANID;
				echo $REDIRECT;
	}
...
else //IF ip address recevied is not Payment Gateway IP Address
{
			...
			*/
			$REDIRECT = 'REDIRECT=http://www.bhatiamobile.com/hdfc/FailedTRAN.php?message=IP ADDRESS MISMATCH';
			echo $REDIRECT;
}
