 ...
 * http://www.php5.net/license/3_0.txt.  If you did not receive a copy of
 ...
 * @license    http://www.php5.net/license/3_0.txt  PHP License 3.0
 * @version    CVS: $Id: DB.php5,v 1.80 2005/02/16 02:16:00 danielc Exp $
 * @link       http://pear.php5.net/package/DB
 */

/**
 * Obtain the PEAR class so it can be extended from
 */
require_once 'sniPEAR'.$G_phpextension;
...
class DB
{
    ...
    function &factory($type, $options = false)
    {
        ...
        if (isset($options['debug']) && $options['debug'] >= 2) {
            ...
            include_once "DB/{$type}$G_phpextension";
        } else {
            @include_once "DB/{$type}$G_phpextension";
        }
        ...
        if (!class_exists($classname)) {
            $tmp = sni_PEAR::raiseError(null, DB_ERROR_NOT_FOUND, null, null,
                                    "Unable to include the DB/{$type}$G_phpextension"
                                    . " file for '$dsn'",
                                    'DB_Error', true);
            return $tmp;
        }
     ...
     *       DB_sybase::connect()
     *
     * @uses DB::parseDSN(), DB_common::setOption(), sni_PEAR::isError()
     */
    function &connect($dsn, $options = array())
    {
        ...
        if (isset($options['debug']) && $options['debug'] >= 2) {
          ...
          //NOTE using
          //       @include_once "DB/${type}.php5";
          //causes any errors to be suppressed...okay in on sense but bad in others. Turn off when debugging
          //
            include_once "DB/${type}$G_phpextension";
        }
 ...
 * @link       http://pear.php5.net/package/DB
 */
class DB_Error extends sni_PEAR_Error
{
    // {{{ constructor
    ...
    {
        if (is_int($code)) {
            $this->sni_PEAR_Error('DB Error: ' . DB::errorMessage($code), $code,
                              $mode, $level, $debuginfo);
        } else {
            $this->sni_PEAR_Error("DB Error: $code", DB_ERROR,
                              $mode, $level, $debuginfo);
        }
    }
