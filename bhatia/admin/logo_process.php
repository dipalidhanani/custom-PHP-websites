...
$column=array("Is_Image","Disp_Order");
...
{ 
...

		{
			...
			{	
				...
				{				  
					...
					$newheight1=$height;//($height/$width)*$newwidth1;
					...
					$pro=mysql_query("select * from logo_mst order by Logo_Id desc limit 1");
					...
					$tfilename = "../order/logo/photo". $b . "." . $extension;
	...
	$value=array($image,$order);
	$txtid=$_POST['txtid'];
	$db->update("logo_mst","Logo_Id",$txtid,$column,$value);
...
{ 
	...
	$value=array($image,$order);
	$db->insert("logo_mst",$column,$value);
}
?>
