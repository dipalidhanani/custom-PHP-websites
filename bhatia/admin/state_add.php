<?php session_start();
	...
	$db->connect();	?>
    <?php
	$country=mysql_query("select * from state where State_Id='".$_REQUEST['eid']."'");
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="10" cellpadding="0">
          <tr>
            <td class="normal_fonts14_black"><a href="state.php">State Details</a></td>
            </tr>
          <tr>
            <td>
            <?php if($_REQUEST['eid']=='') { ?>
            <form name="frmcountry"  method="post" action="state_process.php">
            <?php } else { ?>
            <form name="frmcountry"  method="post" action="state_process.php?is_edit=1">
            <input type="hidden" name="txtid" value="<?php echo $k['State_Id']; ?>" />
            <?php } ?>
            <table width="100%" border="0" cellpadding="5" cellspacing="0" class="border">
              <tr>
                <td width="300" align="right" class="normal_fonts9">Country Name</td>
                <td width="10" align="center" class="normal_fonts9">:</td>
                <td class="normal_fonts9"><select name="country" id="country">
                <?php $co=mysql_query("select * from country order by Name");
				while($c=mysql_fetch_array($co))
				{
				?>
                <option value="<?php echo $c['Country_Id']; ?>" <?php if($k['Country_Id']==$c['Country_Id']) { ?> selected="selected" <?php } ?>><?php echo $c['Name']; ?></option>
                <?php } ?>
                </select></td>
              </tr>
              <tr>
                <td align="right" class="normal_fonts9">State Name</td>
